"""Resource handlers for Persefoni Connector."""
...
@chat.function("list_emissions", "List emissions in Persefoni.", action_type="read", chain_callable=True, event="persefoni-connector.list_emissions", effects=["read:emissions"], data_model=EmissionRecordList)
...
@chat.function("get_emissionrecord", "Get details of one EmissionRecord in Persefoni.", action_type="read", chain_callable=True, event="persefoni-connector.get_emissionrecord", effects=["read:emissionrecord"], data_model=EmissionRecordRecord)
...
@chat.function("audit_emissionrecord_health", "Audit health of Persefoni emissions and connectivity.", action_type="read", chain_callable=True, event="persefoni-connector.audit_emissionrecord_health", effects=["read:audit"], data_model=AuditHealthReport)
async def audit_emissionrecord_health(params: ConnectionIdParams, ctx) -> ActionResult:
    ...
    try:
        ...
        return ActionResult.ok({
            ...
            "summary": f"Persefoni healthy. Sampled {len(items)} emissions."
        }, summary=f"Persefoni health check passed with {len(items)} emissions.")
    except Exception as e:
        return ActionResult.error(f"Error auditing Persefoni health: {e}")
