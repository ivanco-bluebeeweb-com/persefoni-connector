"""Pydantic schemas for Persefoni Connector."""
...
class ConnectParams(BaseModel):
    label: str = Field(default="", description="Friendly connection label, e.g. Primary Persefoni.")
    api_key: str = Field(description="ESG API Key / Client Token")
    base_url: str = Field(default="https://api.persefoni.com/v1", description="Persefoni API base URL.")
...
class GetEmissionRecordParams(BaseModel):
    connection_id: str = Field(default="", description="Optional connection ID.")
    emissionrecord_id: str = Field(description="Persefoni EmissionRecord ID.")
