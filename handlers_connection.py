"""Connection management for Persefoni Connector."""
...
from persefoni_connector_client import PersefoniClient

_SECRET = "persefoni_connector_connections"
...
async def resolve_client(ctx, connection_id: str = "") -> PersefoniClient:
    conns = await _load_conns(ctx)
    if not conns:
        raise ValueError("No Persefoni connections configured. Use connect_persefoni_connector first.")
    conn = conns[0]
    if connection_id:
        for c in conns:
            if c["id"] == connection_id:
                conn = c
                break
    return PersefoniClient(api_key=conn["api_key"], base_url=conn.get("base_url", ""))

@chat.function("connect_persefoni_connector", "Connect Persefoni account via credentials.", action_type="write", chain_callable=True, event="persefoni-connector.connect_persefoni_connector", effects=["create:connection"], data_model=ConnectionRecord)
async def connect_persefoni_connector(ctx, params: ConnectParams) -> ActionResult:
    client = PersefoniClient(api_key=params.api_key, base_url=params.base_url)
    res = await client.verify_auth()
    if res.get("status") == "error":
        return ActionResult.error(f"Failed to connect to Persefoni: {res.get('error')}")
    conns = await _load_conns(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    rec = {
        "id": cid,
        "label": params.label or "Primary Persefoni",
...
async def disconnect_persefoni_connector(ctx, params: ConnectionIdParams) -> ActionResult:
    conns = await _load_conns(ctx)
    if not conns:
