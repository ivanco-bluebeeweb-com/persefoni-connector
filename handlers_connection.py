"""Connection lifecycle for Persefoni Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from persefoni_client import PersefoniClient
...
@chat.function(
    "connect_persefoni",
    "Connect Persefoni account via credentials.",
    action_type="write",
    chain_callable=True,
    event="persefoni-connector.connect_persefoni",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_persefoni(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Persefoni Connector."""
    ...
    record = {
        "id": cid,
        "label": params.label or "Persefoni Account",
...
@chat.function(
    "list_connections",
    "List connected Persefoni accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_persefoni",
    "Disconnect Persefoni account.",
    action_type="write",
    chain_callable=True,
    event="persefoni-connector.disconnect_persefoni",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_persefoni(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
